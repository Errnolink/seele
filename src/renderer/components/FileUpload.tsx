...
export interface FileUploadProps extends MotionSafeDivAttributes {
  ...
  /** Callback when the native folder picker (click-to-browse) resolves. */
  onFolderSelected?: (path: string) => void;
  /** Accepted file types, e.g. ".pdf,.doc" (hidden input). */
  accept?: string;
...
/** Seele palette equivalents of the upstream NERV hues. */
const themeColors = {
  orange: "#ff9830",
  green: "#50ff50",
  cyan: "#20f0ff",
...
/** Mechanical in-out, 150ms — inside the 120–180ms motion band. */
const TWEEN = { duration: 0.15, ease: EASE_MECHANICAL };
...
export const FileUpload = forwardRef<HTMLDivElement, FileUploadProps>(
  function FileUpload(
    ...
    // Seele is a folder scanner: click-to-browse opens the native folder
    // picker (window.scanAPI.selectFolder) instead of a file picker. The
    // hidden input stays for the accept/multiple contract.
    const handleClick = () => {
      if (disabled) {
        return;
      }

      void window.scanAPI.selectFolder().then((path) => {
    ...
    const borderColor = hasError ? "#ff3030" : c;
      ...
      >
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center justify-center py-10 px-6">
            ...
            {/* Sublabel */}
            <span
              className="text-[10px] uppercase tracking-[0.15em] text-nerv-muted"
            ...
            >
              {files.map((file, i) => (
                <motion.li
                  ...
                  exit={{ opacity: 0, x: -8 }}
                  transition={TWEEN}
                  className="flex items-center gap-3 px-3 py-1.5 border border-nerv-border-highlight/20"
                >
                  {/* Dot accent */}
                  ...
                  {/* File name */}
                  <span
                    className="text-[11px] uppercase tracking-[0.1em] text-nerv-text truncate flex-1"
                    style={{ fontFamily: "var(--font-display)" }}
                  >
                    {file.name}
                  </span>
                  ...
                  {/* File size */}
                  <span
                    className="text-[10px] uppercase tracking-[0.1em] text-nerv-muted shrink-0"
                    style={{ fontFamily: "var(--font-display)" }}
                  >
                  ...
                  <button
                    ...
                    className="text-nerv-muted hover:text-nerv-red transition-colors text-xs leading-none cursor-pointer shrink-0"
